...
let main : ∀α, (α → α) × Int =
  Λ α.
    let f : ∀β, β → β = Λ β. app@(List (β → β))@(β → β) head@β ids@β
    in ⟨f@(α → α) id@α, f@Int 1⟩
;; == Runtime ==
(load "runtime.lisp")
...
(defparameter |main-19|
  (let* ((|app-20| (|app-11| (%clos (function |head-4|) 1) |ids-8|))
         (|app-21| (gapply1 |app-20| (%clos (function |id-2|) 1)))
         (|app-22| (gapply1 |app-20| 1)))
     (cons |app-21| |app-22|)))
