...
不适用于 REPL，因为两者在编译过程上有着不同的使用逻辑和性能要求：用户使用交互式的 REPL 时通常
\emph{一步步地}输入程序，但又希望在每一次输入后能立马得到当前的结果。一个最直接的 REPL 可能只是
缓存用户到目前为止输入的所有程序，并在每一次输入之后重新编译\emph{整个程序}——对于具有多阶段的
语言实现来说效率及其低下，尤其是当程序较大时，越往后的输入就需越多无用的重新编译。因而，我们编写了
一套试验性质的增量 API，用于\emph{增量编译}：用户输入一部分程序，我们编译一部分；从不重新编译已经
编译过的部分。
...
且 LISP 家族的语言可以直接看作无类型\lambdacalc（untyped ~, UTLC），
...
因为类型信息已经擦除，我们只能通过按需收集\emph{形状}来\underline{缓解}歧义问题。形状有定义
\begin{minted}{lean4}
...
\subsubsection{本章小结}（..略..）
\section{PP：\emph{依赖类型}的表格打印器}
本文倡导依赖类型编程实践，而最好的倡导方式就是讲解实例。各种实例中，又以
那些具有工程价值，被实际运用于项目中的为贵。因而，我们专门用一部分来介绍 \Tigris 实践中使用的
...
根据 $as$ 的形状 1) 为空；2) 为单个类型；3) 包含两个及以上的类型。
...
之所以使用连续数组 \texttt{Array} 而非列表（链表）\texttt{List}，是因为前者性能更加。此外，虽然表格的列数由
...
依赖类型论语言中都十分重要的机制：\emph{被匹配对象的精练}（discriminant refinement）。\texttt{ToProd}，
...
构成了一套 PL(DI)/定理证明教育解决方案。\Tigrisd 在设计与实现上，受到了 \texttt{Lean}、\texttt{pi-forall}、\texttt{LambdaPi}、
\texttt{elaboration-zoo} 项目的深刻影响。
...
虽然未来展望中，我们希望有一天能将之用 Lean 重写，并配套一个定理证明过（verified）的实现——重点是
当下我们已经拥有一个可以工作的原型。
...
例如一个将长度\emph{嵌入}类型的列表（换句话说，类型依赖长度）定义：
...
这一声明隐式地产生了两条约束，按分支分别为 $n = 0$ 与 $n = Succ m$. 通过 \texttt{example} 关键字，
我们能引入匿名的定义用于测试这个类型定义的可用性：
...
\begin{center}
\underline{函数能接收\emph{类型}作为参数，就像其能接收值一样，因为\emph{值也是项}}。
\end{center}
...
相等，即证 $h : \alpha=\beta$.\footnote{
  依赖类型论的\emph{相等性}是一个极为重要的概念，我们将在下一章给出具体介绍。
...
\item \mintinline{ocaml}|let (x, y) = a in b| 并非上述结构的语法糖，而是专门用于实现
（依赖类型）元组消去的语法构造。
\end{itemize}
...
我们也实现了类 Haskell 文件顶端的声明：通过 \mintinline{haskell}|module ... where| 定义一个模块。
引入模块 $M_1,\dots,M_i$ 可将其中的所有声明引入当前模块，
通过顶端声明 \mintinline{lean4}|import M_1,..,M_i| 实现。
...
类似第二部分，下面我们给出 \Tigrisd 内核的形式化描述。由于类型亦属于项，所以我们不再区分
类型系统和值，统归于核心代数下。
...
\Tigrisd 的双向类型算法存在两个类型判断（judgement）：推断与检查。这两个判断在实现中对应两个
...
$(x : \mathbf{Type})\times (\lambda x.\,\mathsf{Nat})\ \mathsf{Unit}$ is in WHNF,
