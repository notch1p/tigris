...
partial def lowerE (e : Expr) (ρ : Env) (k : Name -> M σ LExpr) : M σ LExpr := do
  ...
  | none =>
    ...
    | .Let x e₁@(.Fix lam) e₂ | .Let x e₁@(.Fixcomb lam) e₂ =>
      match decomposeLamChain lam with
      | some (selfN, params, core) =>
        let funIR : LFun <- lowerRecFun x selfN params core ρ
        let body <- lower e₂ (ρ.insert x x)
        return .letRec #[(x, funIR.param, funIR.body)] body
      | none =>
        lowerE e₁ ρ fun v => do
          let body <- lower e₂ (ρ.insert x x)
          return .letVal x (.var v) body
    | .Let x e₁ e₂ =>
      lowerE e₁ ρ fun v => do
        let body <- lower e₂ (ρ.insert x x)
        return .letVal x (.var v) body
