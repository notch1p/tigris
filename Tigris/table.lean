...
def tiglHelpMsg : TableOf HelpHeader := .mk
  #[ (.str "--tcnf"                 , .str "emit TCNF")
   , (.str ""                       , .str "TCNF: Tigris Compiler Normal Form")
   , (.str "--cc"                   , .str "emit CC'd TCNF")
   , (.str "--opt"                  , .str "emit CC&Optimize'd TCNF")
   , (.str "--sysf"                 , .str "emit System F IR")
   , (.str "--fasl"                 , .str "emit FASL")
   , (.str ""                       , .str "overrides individual <ofiles>")
   , (.str "--speed  n = 3"         , .str "SBCL speed quality")
   , (.str "--safety n = 0"         , .str "SBCL safety quality")
   , (.str "--debug  n = 0"         , .str "SBCL debug quality")
   , (.str "--block-comp"           , .str "Use (disabled by default) block compilation")
   , (.str ""                       , .byl "This option applies to all sources")
   , (.str ""                       , .byl "but is only available for FASL output")
   , (.str "--lam"                  , .str "emit raw lambda IR w/ optmizations")
   , (.str "--lamcc"                , .str "emit CC'd lambda IR")
   , (.str "--lamcps"               , .str "emit CPS IR")
   , (.str ""                       , .byl "--lam* flags only available with --lambda")
   , (.str "-nl, --no-lisp"         , .str "do not emit Common Lisp source")
   , (.str "-ne, --no-entry"        , .str "do not generate entrypoint")
   , (.str "--legacy"               , .str "use deprecated typechecker/IR/backend")
   , (.str ""                       , .byl "not recommended due to a lack of features")
   , (.str "--lambda, --cwc"        , .str "use another deprecated IR/backend")
   , (.str ""                       , .blu "default: TCNF IR with its backend")
   , (.str "-l, --link <path>"      , .str "link additional lisp source from <path>")
   , (.str ""                       , .str "can be used multiple times.")
   , (.str ""                       , .str "⬝ static linking for FASL files")
   , (.str ""                       , .str "⬝ dynamic linking otherwise")
